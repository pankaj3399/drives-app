...
async function addCustomer(req, res) {
  try {
    ...
    if (customer) {
      return res
        .status(400)
        .json({ success: false, message: "Already customer email exists" });
    }
    ...
    if (!newCustomer) {
      return res
        .status(400)
        .json({ success: false, message: "Could not create a new customer" });
    }
  ...
  try {
    ...
    if (!customer) {
      return res.status(400).json({
        success: false,
        message: "Failed to update customer email send status",
      });
    }
...
async function createOrder(req, res) {
  try {
    ...
    if (!order) {
      return res
        .status(400)
        .json({ success: false, message: "Could not create a new order" });
    }
...
async function updateOrderInfo(req, res) {
  try {
    ...
    if (!order) {
      return res
        .status(400)
        .json({ success: false, message: "Failed to update order info" });
    }
    ...
    if (order && completionDate) {
      ...
      if (updatedDrives && user && scans.length) {
        ...
        for (const scan of scans) {
          ...
          if (
            scan.deletionStatus === HardDriveDeletionStatusEnum.FAILED_DELETION
          ) {
            failedDeletion += 1;
          }
        }
...
async function sendEmailToCustomer(req, res) {
  try {
    ...
    const user = await findCustomer({ _id: customerId });
    const scans = await getAllScansSpecificOrder(orderId);
    let drivesDeleted = 0;
    let failedDeletion = 0;
    for (const scan of scans) {
      if (scan.deletionStatus === HardDriveDeletionStatusEnum.DELETED) {
...
async function addNewScan(req, res) {
  try {
    ...
    const checkUniqueSerialNumber = await getAScan({
      serialNumber: req.body.serialNumber,
    });
    if (checkUniqueSerialNumber) {
      return res
        .status(400)
        .json({ success: false, message: "Please enter unique serial number" });
    }
    ...
    if (scans.length >= checkValidOrder.devices) {
      return res.status(400).json({
        success: false,
        message: `Already added ${checkValidOrder.devices} devices`,
      });
    }
    ...
    if (!scan) {
      return res
        .status(400)
        .json({ success: false, message: "Could not add scan device" });
    }
...
async function getScan(req, res) {
  try {
    ...
    if (!serialNumber || serialNumber === "") {
      return res
        .status(422)
        .json({ success: false, message: "Provide a valid serial number" });
    }
    ...
    if (!scan) {
      return res
        .status(400)
        .json({ success: false, message: "Could not get scan device" });
    }
    return res.json({ success: true, data: scan });
  } catch (error) {
    console.log(error);
...
async function updateScanDeletionStatus(req, res) {
  try {
    ...
    const scan = await updateScan(
      { _id: scanId },
      { deletionStatus: status, deletionDate: new Date() }
    );
    if (!scan) {
      return res
        .status(400)
        .json({ success: false, message: "Failed to delete scan device" });
    }
...
async function dashboardData(req, res) {
  try {
    const [customers, scannedDevices, deletedDevices, orders, pendingDevices] =
      await Promise.all([
        ...
        await getScansCount({
          deletionStatus: HardDriveDeletionStatusEnum.NON_STARTED,
        }),
      ]);
    return res.json({
      success: true,
